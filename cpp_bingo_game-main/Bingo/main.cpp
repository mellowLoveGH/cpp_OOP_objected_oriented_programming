...
//	Players[NumPlayers++] = new Bingo_2016000000;	
	
	// initialize score
	for( int i=0 ; i<NumPlayers ; i++ ) SCORE[Players[i]->name()] = 0;
	...
	for( int nmatch = 0 ; nmatch < NUMMATCH ; nmatch++ ) {
		for( int i=0 ; i<NumPlayers ; i++ ) {
			for( int j=i+1 ; j<NumPlayers ; j++ ) {
				...
				// ready
				P1->genMat(BINGO_SIZE); P2->genMat(BINGO_SIZE);
				// play
				while( turn < BINGO_SIZE*BINGO_SIZE ) {
					...
					turn++;
					// mark
					P1->markMine(call1); P2->markMine(call1);
					P1->markMine(call2); P2->markMine(call2);
#ifdef DEBUG
					// show
					cout << P1->name() << endl;
					P1->show();
					...
					if( bingo1 == true && bingo2 == false ) {
						SCORE[P1->name()] += 3; break;	
					} else if( bingo1 == false && bingo2 == true ) {
						SCORE[P2->name()] += 3; break;
					} else if( bingo1 == true && bingo2 == true ) {
						SCORE[P1->name()] += 1; SCORE[P2->name()] += 1; break;
					}
